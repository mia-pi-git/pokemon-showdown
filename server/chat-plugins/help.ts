...
Punishments.roomPunishmentTypes.set('HELPSUGGESTBAN', "Banned from submitting suggestions to the Help filter");
...
export class HelpResponder {
	...
	getRoom() {
		...
		if (!room) throw new Chat.ErrorMessage(`The Helpfilter room is not configured. Set up Config.helpFilterRoom.`);
		return room;
	}
	...
	ban(userid: string, reason = '') {
		const room = this.getRoom();
		const user = Users.get(userid) || toID(userid);
		const punishment: [string, ID, number, string] = ['HELPSUGGESTBAN', toID(user), Date.now() + BAN_DURATION, reason];
	...
	isBanned(user: User | string) {
		const room = this.getRoom();
		return Punishments.getRoomPunishType(room, toID(user)) === 'HELPSUGGESTBAN';
	}
}
	...
	helpfilter: {
		...
		ban(target, room, user, connection, cmd) {
			...
			if (unban) {
				...
				Punishments.roomUnpunish(this.room.roomid, userid, 'HELPSUGGESTBAN');
	...
	helpfilter(args, user) {
		...
		case 'queue':
			...
			for (const request of helpData.queue) {
				...
				const faq = regexString.split('=>')[1];
				buf += `<hr /><strong>FAQ: ${faq}</strong><hr />`;
				buf += `Input: ${regexString}<br />`;
				buf += `Full regex: <code>${Answerer.stringRegex(regexString)}</code><br />`;
				buf += `Suggester: ${userid}`;
