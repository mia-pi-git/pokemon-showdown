...
class OtdHandler {
	...
	constructor(id: string, name: string, room: Room, filename: string, keys: string[], keyLabels: string[], week = false) {
		...
		this.file.read().then((content: string) => {
			...
			if (err.code !== 'ENOENT') throw new Error(err.code);
		});
	}
	...
	addNomination(user: User, nomination: string) {
		...
		if (this.winners.slice(this.room === rooms.jubilifetvfilms ? -15 : -30).some((
			entry: AnyObject
		) => toNominationId(entry[this.keys[0]]) === id)) {
	...
	rollWinner() {
		...
		this.room.add(
			...
			`Out of ${keys.length} nominations, we randomly selected <strong>${winner.nomination}</strong> as the winner! (Nomination by ${winner.name})` +
			`</p><p style="font-weight:bold;">Thanks to today's participants:` + namesHTML
		);
		this.room.update();
	...
	async generateWinnerDisplay() {
		...
		`The ${this.name} of the ${this.timeLabel} is ${winner[this.keys[0]]}${winner.author ? ` by ${winner.author}` : ''}.</span>`;
	...
	generateWinnerList(context: PageContext) {
		...
		for (let i = 0; i < this.keys.length; i++) {
			if (i === 0 || ['song', 'event', 'time', 'link', 'tagline', 'sport', 'country'].includes(this.keys[i]) &&
...
const commands: ChatCommands = {
	...
	removehelp: [
		...
		`Additionally prevents them from voting again until the next round. Requires: % @ # & ~`,
	],
