...
export class Session {
	...
	async getRecentRegistrationCount(period: number) {
		...
		const rows = await users.query(query).then(res => res.rows?.[0]);
		if (!rows) return 0;
		return (rows as any).registrationcount;
	}
	...
	async login(name: string, pass: string) {
		...
		this.session = res.rows?.[0].session || 0;
		return this.dispatcher.user.login(name);
	}
	...
	async checkLoggedIn() {
		...
		const res = await users.query<{sid: string; timeout: number}>(query).then(res => res.rows?.[0]);
